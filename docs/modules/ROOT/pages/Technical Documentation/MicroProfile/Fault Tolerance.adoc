...
NOTE: Users upgrading from Payara Platform Enterprise *6.8.0* (MicroProfile 6.1 support) to Payara Platform Enterprise *TBD* (MicroProfile 7.1 support) will encounter this update, as Fault Tolerance 4.0 (shipped with MicroProfile 6.1) is upgraded to 4.1.

[[fault-tolerance-41-telemetry-metrics]]
...
To enable this, ensure MicroProfile Telemetry is active (`otel.sdk.disabled=false`) and no additional configuration is required. Refer to
xref:Technical Documentation/MicroProfile/Telemetry.adoc[MicroProfile Telemetry documentation] for setup details.
